...
pub struct Contract {
    ...
    //predecessor_account_id + || + TokenID + Sale struct
...
impl Contract {
    ...
    pub fn new(owner_id: ValidAccountId, ft_token_ids:Option<Vec<ValidAccountId>>, bid_history_length:Option<u8>) -> Self {
        ...
        // add all input FT tokens address
    ...
    /// only owner can use this function to add ft tokens
    pub fn add_ft_token_ids(&mut self, ft_token_ids: Vec<ValidAccountId>) -> Vec<bool> {
        // check owner
        self.assert_owner();
        let mut added = vec![];
        // for to add all FT tokens address
        for ft_token_id in ft_token_ids {
    ...
    // return price of every sale
    pub fn storage_amount(&self) -> U128 {
        U128(STORAGE_PER_SALE)
    }

    //return the account storage balance
